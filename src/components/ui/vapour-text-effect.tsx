...
export function VaporizeTextCycle({
  ...
  const lastSetupRef = useRef<{ text: string; font: string; width: number; } | null>(null);
  const particlesRef = useRef<Particle[]>([]);
  const animationFrameRef = useRef<number | null>(null);
  const [currentTextIndex, setCurrentTextIndex] = useState(0);
  const [animationState, setAnimationState] = useState<"static" | "vaporizing" | "fadingIn" | "waiting" | "completed">("static");
  ...
  useEffect(() => {
    // If loop is disabled and we have started vaporizing or completed, do not re-sample/re-render
    if (!loop && animationState !== "static") return undefined;
    // If loop is enabled, only prevent re-sampling during the active vaporization phase
    if (loop && animationState === "vaporizing") return undefined;
    if (animationState === "completed") return undefined;
  ...
  }, [texts, font, color, alignment, wrapperSize, currentTextIndex, globalDpr, transformedDensity, animationState, loop]);
...
const handleFontChange = ({
  ...
  lastSetupRef?: React.MutableRefObject<{ text: string; font: string; width: number; } | null>;
}) => {
  if (lastFontRef.current === null) {
...
const renderCanvas = ({
  ...
  // Check if we can skip setup (if text, font, and width are close enough)
  if (
    lastSetupRef &&
    lastSetupRef.current &&
    lastSetupRef.current.text === currentText &&
    lastSetupRef.current.font === font &&
    Math.abs(lastSetupRef.current.width - width) < 8
  ) {
    return; // Skip setup!
  }
  ...
  const { particles, textBoundaries } = createParticles(ctx, canvas, currentText, textX, textY, font, color, framerProps.alignment || "left", fontSize * globalDpr);
...
const createParticles = (
  ...
  if (color === "gradient") {
    const grad = ctx.createLinearGradient(textLeft, textY - fontSize / 2, textLeft + textWidth, textY + fontSize / 2);
    grad.addColorStop(0, "hsl(225, 100%, 92%)");
    grad.addColorStop(0.45, "hsl(248, 95%, 78%)");
    grad.addColorStop(1, "hsl(195, 90%, 70%)");
    ctx.fillStyle = grad;
  } else {
    ctx.fillStyle = color;
  }
